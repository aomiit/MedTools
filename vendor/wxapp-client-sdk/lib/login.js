            ...
            wx.getUserInfo({
                ...
                fail: function (userError) {
                    ...
                    if(error.detail.errMsg=='getUserInfo:fail auth deny'){
                        console.log('未授权')
              wx.redirectTo({
  url: '/vendor/assets/common/noAuth/notAuth'
})
        }
                    callback(error, null);
                },
    ...
    var doLogin = () => getWxLoginResult(function (wxLoginError, wxLoginResult) {
        ...
        wx.request({
            ...
            success: function (result) {
                ...
                var data = JSON.parse(result.data.replace(/^.*?([\[{])/, "$1"));
                console.log('datalogin返回数据',result)
                // 成功地响应会话信息
                ...
                if (data.success) {
                    ...
                    if (session) {
                        ...
                        var errorMessage = '登录失败：' + (data.message || '未知错误');
...
var setBusinessId = function (businessId) {
    defaultOptions.businessId = businessId;
};
